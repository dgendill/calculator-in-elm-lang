...
# Definition
@docs infixToPrefix, Symbol, Expression, eval, basicEval, showExpr
-}

import List exposing(foldr, indexedMap, append, tail, head, reverse, length, drop)
import Maybe exposing (withDefault)
import List.Extra exposing (lift2, getAt)
...
infixToPrefix : Expression -> Expression
infixToPrefix exp = infixToPrefix_ exp [] []


{-| For testing -}
infixToPrefix_ : Expression -> OpStack -> NewExpression -> Expression
infixToPrefix_ exp opstack nexp =
  let
    symbol = head_ exp
  in
    if (isOperand symbol)
    then infixToPrefix_ (tail_ exp) opstack (symbol :: nexp)
    else
      if (symbol == LParen)
      then infixToPrefix_ (tail_ exp) (symbol :: opstack) nexp
      else
        if (symbol == RParen)
        then
          let (a, b, c) = popStack exp opstack nexp
          in infixToPrefix_ (tail_ a) b c
        else
...
{-| Reduces a postfix expression to a final value (a reduced expression).
infix expression will silently fail. -}
  ...
  then Just (head_ stack)
    ...
    case head_ exp of
      ...
      op    ->
        if (length stack >= 2)
        then
          let result = basicEval (getAt_ 0 stack) (getAt_ 1 stack) op
          in
            if result == Nothing
            then Nothing
